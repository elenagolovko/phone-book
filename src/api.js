...
function acceptAddress(token, container, naviaddress) {
	fetch('https://staging-api.naviaddress.com/api/v1.5/addresses/accept/' + container + '/' + naviaddress, {  
			method: 'post',  
			headers: {  
				"content-type": "application/json",  
				"auth-token": token  
...
.then(response => response.json())  
.then(data => {
	console.log(data); /*информацию о получившемся адресе можно посмотреть в консоли, чисто для нас, для проверки*/
})  
.catch(function (error) {  
	console.log('Request failed', error);  
});
